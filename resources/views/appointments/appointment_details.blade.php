...
<div class="min-h-screen bg-gradient-to-br from-cyan-50 via-sky-50 to-blue-50 p-6 mt-24">
    ...
    <!-- Header -->
        <div class="flex items-center gap-4 mb-8 bg-white/80 backdrop-blur-xl rounded-2xl shadow-lg border border-white/20 p-6">
            <a href="{{ route('appointment.index') }}" class="inline-flex items-center justify-center w-10 h-10 rounded-full bg-cyan-100 text-cyan-700 hover:bg-cyan-200 transition-all">
    ...
    <!-- Main Content -->
        ...
        <!-- Patient Section -->
            <div class="bg-white/80 backdrop-blur-xl rounded-2xl shadow-lg border border-white/20">
                <div class="flex items-center justify-between p-6 border-b border-cyan-100">
                    ...
                    <a href="{{ route('patient.show',$appointment->patient->id) }}" class="inline-flex items-center gap-1 text-cyan-600 hover:text-cyan-800 font-medium transition-all">
        ...
        <!-- Coach Section -->
            <div class="bg-white/80 backdrop-blur-xl rounded-2xl shadow-lg border border-white/20">
                <div class="flex items-center justify-between p-6 border-b border-cyan-100">
                    <div class="flex items-center gap-3">
                        <i class="fas fa-user-md text-cyan-500 text-xl"></i>
                        <h2 class="text-xl font-semibold text-gray-800">Détails du Coach</h2>
                </div>
                    <a href="{{ route('user.show',$appointment->coach->id) }}" class="inline-flex items-center gap-1 text-cyan-600 hover:text-cyan-800 font-medium transition-all">
                        <span class="hidden sm:inline">Voir le Profil</span> <i class="fas fa-external-link-alt"></i>
                </a>
                </div>
                <div class="p-6 space-y-3">
                    <div><span class="font-semibold text-gray-700">Nom :</span> {{ $appointment->coach->full_name }}</div>
                    <div><span class="font-semibold text-gray-700">Spécialité :</span> {{ $appointment->speciality->name }}</div>
                </div>
            </div>
        <!-- Schedule Section -->
            <div class="bg-white/80 backdrop-blur-xl rounded-2xl shadow-lg border border-white/20">
        ...
        <!-- Report Section -->
            <div class="bg-white/80 backdrop-blur-xl rounded-2xl shadow-lg border border-white/20">
                    ...
                    <!-- Existing Report -->
                        <div class="flex flex-col md:flex-row items-center justify-between gap-4 bg-cyan-50 rounded-xl p-4">
                            ...
                            <div class="flex gap-2">
                                <a href="{{ route('appointments.downloadReport', $appointment->id) }}" class="inline-flex items-center justify-center w-10 h-10 rounded-lg bg-blue-100 text-blue-700 hover:bg-blue-200 transition-all" title="Download">
                                <i class="fas fa-download"></i>
                            </a>
                                <a href="{{ route('appointments.viewReport', $appointment->id) }}" target="_blank" class="inline-flex items-center justify-center w-10 h-10 rounded-lg bg-green-100 text-green-700 hover:bg-green-200 transition-all" title="View">
                                <i class="fas fa-eye"></i>
                            </a>
                                <form action="{{ route('appointments.deleteReport', $appointment->id) }}" method="POST" class="inline">
                                @csrf
                                @method('DELETE')
                ...
                @else
                    <!-- Upload Form -->
                        <form action="{{ route('appointments.uploadReport', $appointment->id) }}" method="POST" enctype="multipart/form-data" id="report-form">
                        @csrf
        ...
        @if ($appointment->status == 'pending')
                <div class="flex flex-col md:flex-row gap-4 justify-center mt-8">
                    <a href="{{ route('appointment.edit',$appointment->id) }}" class="inline-flex items-center justify-center px-6 py-3 rounded-xl bg-red-100 text-red-700 font-semibold hover:bg-red-200 transition-all">
